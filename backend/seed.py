...
for t in ['invoice_residents','daily_help_apartments','daily_help','lease_extensions','move_documents','move_requests','marketplace_posts','delivery_apartments','visitor_entries','visitor_invitations','payments','invoices','bookings','spaces','pets','vehicles','documents','move_doc_requirements','document_requirements','otp_sessions','user_active_context','residents','push_tokens','user_society_roles','users','apartments','towers','society_banners','advertisements','society_news','societies']:
...
for uid2,ph,nm,em in [
    (admin1,'9999900001','Admin Sharma','admin@gv.com'),
    (admin2,'9999900003','Admin Desai','admin@sh.com'),
    (guard1,'9999900002','Raju Guard',None),
    (guard2,'9999900004','Mohan Guard',None),
    (u1,'9876543210','Rahul Mehta','rahul@email.com'),
    (u2,'9876543211','Priya Patel','priya@email.com'),
    (u3,'9876543212','Amit Kumar','amit@email.com'),
    (u4,'9876543213','Sneha Reddy','sneha@email.com'),
    (u5,'9876543214','Vikram Singh','vikram@email.com')]:
    c.execute("INSERT INTO users VALUES(?,?,?,?,CURRENT_TIMESTAMP)", (uid2,ph,nm,em))

# user_society_roles: id, user_id, society_id, role, guard_pin, guard_name, created_at
for u,s,r in [
    (admin1,s1,'admin'),(admin1,s1,'super_admin'),
    (admin2,s2,'admin'),
    (guard1,s1,'guard'),(guard2,s2,'guard'),
    (u1,s1,'resident'),(u2,s1,'resident'),(u3,s1,'resident'),(u4,s1,'resident'),
    (u1,s2,'resident'),(u5,s2,'resident')]:
    pin  = '1234' if (r=='guard' and u==guard1) else ('5678' if (r=='guard' and u==guard2) else None)
    name = 'Raju Guard' if u==guard1 else ('Mohan Guard' if u==guard2 else None)
    c.execute("INSERT INTO user_society_roles VALUES(?,?,?,?,?,?,CURRENT_TIMESTAMP)", (uid('ur-'),u,s,r,pin,name))

# residents: id,user_id,apartment_id,society_id,resident_type,status,invited_by,lease_start,lease_end,rejection_reason,created_at
c.execute("INSERT INTO residents VALUES(?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", ('res-001',u1,'apt-A101',s1,'owner','approved',None,None,None,None))
c.execute("INSERT INTO residents VALUES(?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", ('res-002',u2,'apt-A102',s1,'owner','approved',None,None,None,None))
c.execute("INSERT INTO residents VALUES(?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", ('res-003',u3,'apt-B101',s1,'tenant','approved',None,'2024-01-01','2025-12-31',None))
c.execute("INSERT INTO residents VALUES(?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", ('res-004',u4,'apt-A201',s1,'family_member','approved',u1,None,None,None))
c.execute("INSERT INTO residents VALUES(?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", ('res-005',u1,'apt-C101',s2,'owner','approved',None,None,None,None))
c.execute("INSERT INTO residents VALUES(?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", ('res-006',u5,'apt-C102',s2,'owner','approved',None,None,None,None))
c.execute("INSERT INTO residents VALUES(?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", ('res-007',u5,'apt-A301',s1,'tenant','pending',None,'2026-01-01','2026-12-31',None))

for u,a,s in [(u1,'apt-A101',s1),(u2,'apt-A102',s1),(u3,'apt-B101',s1),(u4,'apt-A201',s1),
              (u5,'apt-C102',s2),(admin1,None,s1),(admin2,None,s2),(guard1,None,s1),(guard2,None,s2)]:
...
# invoices: id,booking_id,user_id,society_id,amount,title,description,invoice_type,due_date,notes,raised_by,status,created_at
c.execute("INSERT INTO invoices VALUES(?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", ('inv-001','bk-001',u1,s1,2000,'Community Hall Booking','Community Hall - Mar 15','booking',None,None,None,'paid'))
...
for did,si,rt,dt in [
    ('dreq-001',s1,'owner','property_papers'),('dreq-002',s1,'owner','government_id'),
    ('dreq-003',s1,'tenant','rent_agreement'),('dreq-004',s1,'tenant','government_id'),
    ('dreq-005',s1,'owner_family','government_id'),('dreq-006',s1,'tenant_family','government_id'),
    ('dreq-007',s2,'owner','property_papers'),('dreq-008',s2,'owner','government_id'),
    ('dreq-009',s2,'tenant','rent_agreement'),('dreq-010',s2,'tenant','government_id'),
    ('dreq-011',s2,'owner_family','government_id'),('dreq-012',s2,'tenant_family','government_id')]:
...
print('Seed complete!')
print('Logins: 9876543210(Rahul) 9876543211(Priya) 9876543212(Amit) 9999900001(AdminGV) 9999900003(AdminSH)')
print('Guard PINs: Green Valley=1234  Sunrise Heights=5678')
print('OTP: 123456')
